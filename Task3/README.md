...
⚠️ **Важное отличие для OFAC:** если недоступен именно OFAC-сервис (а не ML), это НЕ Availability Escalation с fallback на "разрешить" — это fail-closed: операция уходит в expert_queue, а не авто-одобряется. Fail-open на санкционной проверке — самая опасная ошибка на границах доверия сервиса (подробнее — threat_model.md).
...
### 🛡️ Угрозы на границах доверия

Схема выше рабочая: понятно, что происходит в обычном случае, куда уходят неоднозначные операции и как сервис переживает отказ модели. Отдельный вопрос — что происходит на четырёх границах, где сервис пересекается с внешним миром: **Frontend**, **OFAC**, **Counterparty Risk DB**, **Expert Queue**.
...
Полный разбор (кто может передать данные под видом легитимного источника, что может подменить решение, чем закрывается) — в **threat_model.md**.
...
| Дешево ли? | Да, 1.711M ₽/месяц vs 2.25M ₽/месяц (ручной), ROI 28% Год 1 — детали в Task4 | Low |
...
| **threat_model.md** | Угрозы на 4 границах доверия (Frontend, OFAC, Risk DB, Expert Queue) | ✅ |
...
| **Fallback strategy** | Rules-only (fail-closed на OFAC) | Если ML упадет — правила; если упал OFAC — expert_queue, не auto-allow |
| **Cost (ML system, полный TCO)** | 1.711M ₽/мес | Инфра + эксперты + ML-support, детально в Task4/cost_analysis.md |
| **Cost (manual, baseline)** | 2.25M ₽/мес | 25 экспертов × 80K + overhead 250K |
| **Savings** | 539K ₽/мес | Год 1 ROI 28%, Год 2+ ROI 32% — Task4/roi_calculation.md |
