...
| Cost | 2.25M ₽/мес | 1.711M ₽/мес | 24% снижение (полный расчёт TCO — в Task4/cost_analysis.md) |

---

## Угрозы на границах доверия

Схема выше показывает happy path, эскалацию и fallback внутри системы. Но у сервиса есть четыре точки, где он пересекает границу доверия с внешним миром: Frontend, OFAC API, Counterparty Risk DB и Expert Queue (люди). Для каждой границы разобрано, кто может выдать себя за легитимный источник, что может подменить решение по пути, и чем это закрывается — см. **threat_model.md**.

Две угрозы там отмечены как приоритетные: fail-open при недоступности OFAC (тихо превращает контроль в декоративный) и сговор эксперта на границе Expert Queue (использует легитимный workflow как маскировку).

---

**Next:** Component Design, Monitoring Plan, ADR Architecture, Threat Model.
